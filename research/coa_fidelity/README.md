...
11. **Update `STATUS.yaml`** for this jurisdiction: `fidelity_status:
    verified` (or `blocked`, if step 5 dead-ended — see "Blocked
    jurisdictions" below), official/current code counts, source URL +
    authority, `last_verified` date, notes. Then regenerate
    `research/coa_fidelity/BLOCKED.md`:
    `python3 scripts/coa_fidelity/build_blocked_report.py` — cheap,
    idempotent, read-only w.r.t. STATUS.yaml, so just always run it after
    touching STATUS.yaml regardless of this round's outcome (it also picks
    up count corrections even on a `verified` round).
...
## Blocked jurisdictions — retry queue

`fidelity_status: blocked` means a round was attempted and hit a real
obstacle (most often: this environment's outbound-HTTPS egress policy
rejects the primary-source host — an environment/tooling limitation, not
proof the source doesn't exist). A `blocked` row is never picked as "the
next jurisdiction" by step 3 (that only pulls `partial_curated_subset`
rows), so blocked jurisdictions silently fall out of the normal per-round
rotation unless something explicitly re-surfaces them. That something is:

- **`research/coa_fidelity/BLOCKED.md`** — auto-generated by
  `scripts/coa_fidelity/build_blocked_report.py` from `STATUS.yaml`. One
  place listing every blocked jurisdiction with its full notes (candidate
  sources tried, exact blocker evidence, retry guidance) and a summary
  table. Regenerated as part of step 11 above every round, so it never
  drifts out of sync with `STATUS.yaml`. **Never hand-edit it** — rerun the
  script.
- Do not treat a growing `BLOCKED.md` as sweep debt to panic over — it's
  the point of the file: it makes blocked jurisdictions visible and
  revisitable instead of silently lost. Retrying one is a normal round (an
  explicit-ISO invocation of step 3, e.g. "do the next round for `dz`"),
  not a special procedure — the only difference is you're allowed to pick
  a `blocked` row instead of the next `partial_curated_subset` one.
- **At sweep close** (see below), walk `BLOCKED.md` top to bottom and
  attempt a retry round for each row before finalizing — a jurisdiction
  that's blocked purely on tooling should not silently ship as
  unresolved if a working-egress attempt could have cleared it.
...
1. Walk `research/coa_fidelity/BLOCKED.md` and retry each blocked
   jurisdiction (see "Blocked jurisdictions" above) — confirm egress
   actually works this time before re-attempting, and either clear it to
   `verified` or update its `last_checked`/notes with the new attempt's
   evidence. Do this *before* step 2 so the final counts below are accurate.
2. Run `scripts/mass_consolidation_v2.py` once on the sweep trunk to get the
   final entity/country/coverage-pct/nodes-hit numbers.
3. Run `scripts/build_jurisdiction_manifest.py` for the final
   statutory/Tier-1 counts.
4. Update all four citable surfaces, `.github/workflows/ci.yml`'s expected
   block, and `tests/test_surface_claims.py`'s REQUIRED/BANNED lists in one
   commit.
5. Open the one PR from `claude/coa-fidelity-sweep` into `main` for
   Christian's review — this is the only point `main` moves.

## Status

See `STATUS.yaml` for live counts, `BLOCKED.md` for the retry queue.
Ecuador is the pilot (round 1, PR #78).
