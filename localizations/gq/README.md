# Equatorial Guinea (`gq`) localization

**Status: verified against primary source (2026-07-16), via the shared
SYSCOHADA chart family.** Equatorial Guinea is one of the 17 OHADA member states bound
...
This directory already carried a pre-fidelity-sweep `gq_mapping.yaml` (a
...
unchanged**, because `logic/knowledge_base.py` reads it to register Equatorial Guinea
...
is now the source of truth for Equatorial Guinea's chart of accounts, and
`gq_mapping.yaml`'s stale codes/UUIDs should not be trusted or extended.
...
- `core/schemas/chart_families.yaml` (`families.SYSCOHADA`) -- the 33
  ...
  engine indexes for Equatorial Guinea (and all 16 other members) via
...
See `research/coa_fidelity/STATUS.yaml`, row `iso: gq` --
