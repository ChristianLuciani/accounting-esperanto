# Mali (`ml`) localization

**Status: verified against primary source (2026-07-16), via the shared
SYSCOHADA chart family.** Mali is one of the 17 OHADA member states bound
...
This directory already carried a pre-fidelity-sweep `ml_mapping.yaml` (a
...
unchanged**, because `logic/knowledge_base.py` reads it to register Mali
...
is now the source of truth for Mali's chart of accounts, and
`ml_mapping.yaml`'s stale codes/UUIDs should not be trusted or extended.
...
- `core/schemas/chart_families.yaml` (`families.SYSCOHADA`) -- the 33
  ...
  engine indexes for Mali (and all 16 other members) via
...
See `research/coa_fidelity/STATUS.yaml`, row `iso: ml` --
