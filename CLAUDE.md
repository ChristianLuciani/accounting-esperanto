...
- **FX is auditable per transaction (`FXQuote`).** Every conversion produces an attachable `FXQuote(currency, usd_per_unit, source, mode, as_of, retrieved_at, note)` — `source` ∈ `frankfurter-ecb` | `open-er-api` | `static-pinned` | `manual`; `as_of` is the upstream publication date (ECB `date` / ExchangeRate-API `time_last_update_utc`). The engine stamps the quote on each `ResolvedEntry.fx` and exposes `ConsolidationResult.fx_quotes` (per subsidiary); the REST `/consolidate` response includes an `fx_audit` array. **Manual input for asynchronous transactions** (historical/contract rates) is first-class and equally audited: `SubsidiaryTB.fx_rate_to_usd` + `fx_rate_as_of` + `fx_rate_note` (engine) and `TrialBalance.exchange_rate` + `exchange_rate_as_of` + `exchange_rate_note` (REST) → `manual_quote(...)`. Providers expose `.quote()`; the chain returns the answering provider's provenance.
...
4. **API-first at the data layer, agent-native at the access layer** — the canonical interface is a machine-consumable API (REST/gRPC). On top of it, Kontablo exposes an agent-native layer for the agentic economy via MCP (Model Context Protocol, for LLM/agent tool consumption), A2A (Agent2Agent, for agent-to-agent interoperation), and AP2 (Agent Payments Protocol, for settlement). The API is the body; the agent protocols are the face. An ERP or bank consumes the API directly; an autonomous agent consumes the agent-native layer. Do NOT subordinate one to the other. The agent-native layer is protocol-pluggable by design: Kontablo will adopt and add any agentic protocol that gains meaningful traction, so the architecture names the *category* (agent-native) rather than betting on a single protocol. **Implementation status (June 2026):** REST is fully implemented (FastAPI); gRPC is defined in `api/grpc/kontablo.proto` with a minimal server (`api/grpc/server.py`) implementing only the **deterministic** RPCs (account queries, mapping, consolidation+intercompany elimination, balance validation) over the same engine — LLM-dependent RPCs return `UNIMPLEMENTED`. Do not describe gRPC as having full feature parity with REST; describe it as "deterministic core implemented, remainder planned." The **MCP deterministic core is now implemented** too (`api/mcp/server.py`, official `mcp`/FastMCP SDK, stdio transport): five tools — `resolve_account`, `get_account` (by id or UUID), `validate_balance_sheet`, `consolidate_trial_balances` (with intercompany elimination), `list_jurisdictions` — each mirroring a deterministic gRPC RPC over the **same** `core.harness`/`core.engine` brain (no logic duplicated). The Tier-3 semantic/LLM fallback is deliberately **not** exposed as an MCP tool (it would hide stochasticity); `resolve_account` escalates to human review with `resolved=false` rather than guessing. Same honesty bar as gRPC: describe MCP as "deterministic tools implemented, Tier-3/LLM tools planned" — **not** full feature parity with REST. A2A and AP2 remain asserted-but-unimplemented; do not claim otherwise. **Input hardening (resilience) — shared across all three faces.** The monetary-input invariants now live in one place, `core/harness/validation.py` (`ensure_finite`, `ensure_positive_finite`, `is_finite_number`), and back REST, gRPC, **and** MCP so they cannot disagree at the boundary: monetary amounts must be finite (NaN/±Inf rejected; a NaN would "balance" since `abs(NaN)>tol` is False, and is invalid JSON), manual FX rates must be `>0` and finite (a 0 zeroes every amount, a negative sign-flips them; in gRPC/proto3 a `0.0` scalar is "unset" and falls back to the provider, a negative is rejected), and a finite-sum overflow is surfaced as a clean error rather than emitted as `inf`. Rejection is face-appropriate: REST `422` (amount/FX) / `400` (overflow, unpriceable currency) — **with a `RequestValidationError` handler that scrubs non-finite values from the 422 body, or FastAPI's input-echo crashes serialization (Starlette `allow_nan=False`) into a 500**; gRPC `INVALID_ARGUMENT`; MCP structured `ToolError` (server stays alive). MCP also validates `nature` (`debit`/`credit` or omitted) and self-describes every tool/param in its schema; negatives are allowed everywhere (contra entries). The deterministic REST `/consolidation` now also reports `balanced`/`balance_difference` (parity with gRPC/MCP). Tests: `tests/test_validation.py` (shared helpers), `tests/api/test_robustness.py` (REST 4xx not 500), `tests/grpc/test_grpc_smoke.py` (INVALID_ARGUMENT), `tests/mcp/test_mcp_server.py` (hermetic, keyless). The MCP server is **core** → BSL 1.1 (not the Apache connectors' license).
...
| Test suite state | `python -m pytest tests/` (155 passed, 1 skipped, 0 failed as of June 2026 — the `tests/mcp/test_mcp_server.py` MCP suite contributes 36, and the API-robustness round adds `tests/test_validation.py` (shared finite/positive-FX invariants) + `tests/api/test_robustness.py` (REST returns 4xx, never a 500, on non-finite amounts / bad FX / overflow) + gRPC INVALID_ARGUMENT regressions; the lone skip is the FX-live network test in `test_fx_provider.py`, gated on `KONTABLO_FX_LIVE_TEST=1`. The Tier-3 LLM-integration tests in `tests/microsaas/test_mapping_api.py` are `skipif` on provider-key absence — they pass when a GROQ/CEREBRAS/GOOGLE_AI/OPENROUTER key is reachable via env or Infisical and skip in a keyless CI, so a keyless run shows additional skips. `pytest tests/ connectors/` is the CI command and adds the connector suites + the surface-drift gate → 169 passed, 1 skipped) |
...
| 0 silent losses / exact entry-level reconstruction (441 entries, 75 entities, 68 jurisdictions) | `python scripts/roundtrip_audit.py` → `research/experiments/roundtrip_audit/results.json` (ADR-016; CI-gated). The claim is: every input row is conserved, every local trial balance reconstructs byte-for-byte from lineage alone, and every non-translation is a typed loss-ledger record. **The projection stays lossy (L3, deliberate coarsening); the SYSTEM is lossless — never blur that distinction.** |
