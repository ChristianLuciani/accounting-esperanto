...
\node[agent] (agent) at (-4.8,3.55)
  {Autonomous agent\\[1pt]
   {\scriptsize\mdseries settles over AP2 / A2A;\\ no human at transaction latency}};
...
\node[ledger, fill=white, draw=clappsInk!55] (human) at (7.0,-8.35)
  {\textbf{Accountable human} --- authorizes the deterministic
   policy \emph{ex ante}, disposes of this counted residual
   \emph{asynchronously}, and can revoke a \emph{rule} rather
   than a transaction. Legal principal in both modes};
...
\emph{not} saying: the human is absent from transaction latency, not from
authority. They authorize the deterministic policy in advance, work the counted
residual asynchronously, and --- because every resolution names the rule that
produced it --- can revoke a rule rather than a transaction
