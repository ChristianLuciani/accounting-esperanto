...
wrong. What becomes impossible is human review \emph{at transaction latency, once
per transaction}. Human \emph{authority} does not become impossible, and it does
not become optional: the human remains the legally accountable principal, and
nothing in this paper transfers accountability to an agent. What changes is
\emph{when} that authority is exercised and \emph{over what unit}. We return to
this in Section~\ref{sec:coresponsibility}, because a paper that removed the
human from the loop without saying what the human then does would be describing
an unaccountable system, not an auditable one.
...
\begin{itemize}
  ...
  review does not scale with volume at all.
\end{itemize}
...
Two limits, so the claim is not read as more than it is. First, the
\emph{records} that would support rule-level revocation are complete --- the
\texttt{rule\_id} is stamped on every resolution and surfaced on every face ---
but no query surface exposes them by rule: \texttt{lineage()}
(\pathtt{core/engine.py}) groups resolved entries by consolidated line, not by
the rule that produced them. Retrieving every posting a given rule produced is
therefore possible from the committed audit data and is \emph{not} implemented as
an API. We claim the provenance, not the tooling. Second, no
review-queue workflow is part of this contribution; the loss-ledger records exist
and are counted, and what an organization does with them is outside what is
demonstrated here. Section~\ref{sec:threat} states the corresponding attack: an
agent can inflate that queue faster than it is worked.
