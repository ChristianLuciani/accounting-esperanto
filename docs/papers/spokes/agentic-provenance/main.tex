...
\begin{abstract}
\noindent Kontablo is a graph-based, UUID-keyed universal accounting ontology: a
deterministic semantic-resolution layer between local statutory charts of
accounts and the machine-to-machine (M2M) agentic economy. We ask a question
specific to autonomous multi-agent finance: as agents run
high-frequency transactions over protocols such as AP2 and A2A, \emph{what
deterministic auditability invariants must the semantic-resolution layer satisfy
for those transactions to be trustworthy without a human in the per-transaction
critical path?} The answer cannot be procedural --- per-transaction review does
not scale --- but architectural. We name three invariants and demonstrate them
in committed code: (\textbf{I1})
\emph{ontology-as-constraint} --- the agent-facing interface exposes only
deterministic tools, none able to emit an identifier not in the graph;
(\textbf{I2}) the \emph{loss ledger} --- everything the layer cannot translate
with certainty becomes a typed, counted record that escalates, never a silent
guess; (\textbf{I3}) the \emph{pre-transaction fiber query} --- an agent can
retrieve the exact preimage of any consolidated node \emph{before} acting,
making auditability a precondition rather than a forensic afterthought. Removing
the human from per-transaction latency is not removing the human: the invariants
let an accountable human authorize policy \emph{ex ante}, dispose of a counted
residual asynchronously, and revoke a \emph{rule} rather than a transaction ---
one decision then governs every posting that rule produced. We
instantiate the invariants over an MCP surface of six deterministic tools and a
round-trip audit that, over $441$ ledger entries from $75$ synthetic entities in
$68$ jurisdictions, reconstructs every local trial balance byte-for-byte from
lineage alone and records \emph{zero} silent losses. The contribution is a shape
for machine-verifiable trust at the resolution layer, beneath and complementary
to settlement-layer protocols.
...
It is worth being precise about what that does and does not remove, because the
imprecise version of this argument is the one that gets Kontablo's own thesis
wrong. The distinction that matters is between an \emph{authority} and its
\emph{exercise}. What machine frequency makes impossible is the exercise of human
review once per transaction, at transaction latency. It makes nothing whatever
impossible about the authority: the human remains the legally accountable
principal, retains the power to pause, inspect, slow, or reverse, and that power
is not suspended at any layer --- including the layer where resolutions are being
applied automatically. That it is not normally exercised on a given transaction is
a fact about normal operation, not a gap in the human's standing.

We are explicit about this because the usual shorthand --- ``human out of the
loop'' --- names a different and considerably worse system than the one described
here, and because a paper that removed the human without saying what the human
then does would be describing something unaccountable rather than something
auditable. Section~\ref{sec:coresponsibility} states what the human does, and
argues that the three invariants are precisely what make that authority
exercisable asynchronously instead of not at all.

The same premise relocates a hard problem. If no human verifies each mapping at
...
(verifiable, reproducible) rather than a procedural one (per-transaction human
review). The accountable human does not disappear --- their authority moves to
policy, to the counted residual, and to the rule
(Section~\ref{sec:coresponsibility}). We give three invariants that make this
so, and a committed audit that exhibits them.}
...
\input{figures/fig_agent_decision_flow}
...
\input{figures/fig_fiber_query}

\subsection{What does the accountable human do at machine frequency?}
\label{sec:coresponsibility}

The three invariants are usually read as verifiability properties. That reading
is incomplete, and the incomplete reading is dangerous, because it invites the
conclusion that the human has been engineered out. The opposite is the case: the
invariants exist so that human authority can be exercised \emph{asynchronously}
rather than not at all. This subsection says what the human actually does, since
a system that removed the human from the loop without answering that question
would be unaccountable rather than auditable.

The case study's governing framework --- its Co-responsibility Architecture ---
resolves this with two authorization modes and one accountable
human~\cite{kontablo_hub}. Deterministic resolutions post under \emph{standing
ex-ante authorization}: the responsible accountant approves the deterministic
policy once --- the code maps and the boundary rules --- configures it, and
retains veto and the right to inspect or reverse any posting it produced. These
postings are logged and fully reviewable but are not individually signed, exactly
as an accountant today authorizes a recurring posting rule without re-signing
each instance it fires. Only the residual --- escalations, boundary flags,
overrides --- requires \emph{per-exception disposition}. The accountant's
signature is therefore two records rather than one: a standing authorization of
the policy, and a sign-off on each flagged item. The review \emph{pathway} and
the veto exist for every transaction; the \emph{act} of review is spent on the
exceptions instead of rubber-stamping the deterministic majority. Legal
accountability is undivided and human in both modes.

Standing authorization is revocable at will, and that is what keeps it an
authorization rather than an abdication. The accountable human can withdraw it
from a rule, a jurisdiction, or an ontology node at any moment
(\texttt{ReviewPolicy}, \pathtt{core/harness/oversight.py}); matching resolutions
then stop being applied and come back with \texttt{tier="held"} and a
\texttt{rule\_id} of \texttt{held:<scope>:<the rule that was withheld>}. A hold is
deliberately \emph{not} an escalation: an escalation means no deterministic answer
existed, a hold means one existed and a human decided it must not be applied
unreviewed. Both are typed and counted, so neither is silent (I2). The point of
mentioning a mechanism this small in a paper is what its existence establishes:
the power to interrupt automatic classification is continuous, and the normal
case --- not exercising it --- is a choice rather than a limitation.

That capability is also, deliberately, \emph{not} on the agent-facing tool
surface. An agent must not be able to place or release a hold, or revoke a rule;
that would hand the constraint to the party the constraint exists to bound. I1
narrows what the agent may ask for, and the same reasoning narrows what the agent
may ask \emph{of the oversight mechanism} --- namely nothing. The six deterministic
tools remain the whole of the agent's surface.

What this paper adds is the mechanism that makes the asynchronous mode
\emph{tractable}, and the three invariants map onto the three tenses of that
authority:

\begin{itemize}
  \item \textbf{Ex ante --- authorizing the policy (I3).} A standing
  authorization is only meaningful if the person granting it can see what they
  are authorizing. The fiber query is what makes the ex-ante decision informed
  rather than nominal: before approving a policy, the accountant can retrieve the
  exact preimage of any universal node --- which local statutory codes collapse
  into it, in which jurisdiction, with what local structure --- and decide
  whether that coarsening is acceptable for their entity. Without I3, standing
  authorization would be consent to something unseen.

  \item \textbf{Continuously, asynchronously --- disposing of the residual
  (I2).} A queue can only be worked if it is bounded and typed. The loss ledger
  is exactly that: every non-translation is a counted record of a known kind ---
  $4$ escalations, $14$ placeholders, $0$ collisions, $2$ compliance flags on the
  dataset of Section~4 --- so the human's workload is a function of the
  \emph{residual}, not of transaction volume. This is the quantitative form of
  the argument: at $0.9\%$ escalation, one reviewer's attention covers a
  transaction stream that per-transaction review could not. An unbounded ``check
  everything'' obligation is what actually excludes a human; a counted queue is
  what restores them.

  \item \textbf{Ex post --- revoking and correcting (I1 plus provenance).} This
  is where the unit of human decision changes, and it is the part we consider
  most consequential. Because every resolution carries the exact rule identifier
  that produced it (\texttt{tier1:<jur>:<code>} or
  \texttt{tier2:<node>:<keyword>}), the accountable human's object of judgement
  is no longer the individual transaction but \emph{the rule}. Concluding that a
  keyword rule was wrong is a single decision that determines the disposition of
  every posting that rule ever produced, and of every posting it would produce
  in future. Per-transaction review scales linearly with volume; rule-level
  review does not scale with volume at all. Operationally this is
  \texttt{by\_rule()} (\pathtt{core/engine.py}) --- the dual of
  \texttt{lineage()}, grouping resolved entries by the rule that produced them
  rather than by the line they aggregated into --- and
  \texttt{revocation\_impact()}, which reports how many postings, nodes and
  jurisdictions a revocation would touch \emph{before} it is applied. It reports
  and does not mutate: applying a revocation is a separate, human-authorized act.
\end{itemize}

So the honest summary is not that architecture replaces the human. It is that
architecture changes the human's unit of work from the transaction to the policy
and the exception --- which is a \emph{gain} in the reach of a single human
decision, obtained precisely by making resolution deterministic and its
provenance total. A stochastic resolver could not offer this: there would be no
stable rule identifier to revoke, and therefore no class of postings to correct
as a class.

Two limits, so the claim is not read as more than it is. First, what is
implemented is the \emph{mechanism} --- the hold, the rule-level grouping, the
revocation-impact report, each covered by tests --- not an operator product. There
is no review-queue workflow, no user interface, and no persistence layer for the
policy: a deployment supplies those. The loss-ledger records exist and are
counted; what an organization does with them is outside what is demonstrated here.
Second, holding and revoking bound the damage a wrong rule can do, but neither
makes the resolver's judgement correct. A rule can be right for every entity but
one, and the mechanism has no way to know that --- it enforces a human's decision,
it does not supply one. Section~\ref{sec:threat} states the corresponding attack:
an agent can inflate the review queue faster than a human works it, and neither
the hold nor the ledger prevents that.
...
\begin{itemize}
  ...
  \item Human oversight (Section~\ref{sec:coresponsibility}):
  \texttt{ReviewPolicy}, \texttt{postings\_by\_rule} and
  \texttt{revocation\_impact} in \pathtt{core/harness/oversight.py}, plus
  \texttt{ConsolidationResult.by\_rule()} in \pathtt{core/engine.py}; tests in
  \pathtt{tests/test\_oversight.py}. Deliberately absent from
  \pathtt{api/mcp/server.py}: the agent surface stays at six tools. The default
  (no policy) holds nothing, so the numbers above are unaffected --- both
  \texttt{results.json} artifacts regenerate byte-for-byte with the oversight
  code present.
