...
\begin{tikzpicture}[
    ...
    flagnode/.style={draw=red!60!black, rectangle, rounded corners=4pt,
        fill=red!10, text width=4.0cm, align=center,
        minimum height=1.2cm, font=\small},
    dbnode/.style={cylinder, shape border rotate=90, aspect=0.3,
        draw=gray!70!black, cylinder uses custom fill,
        cylinder body fill=gray!16, cylinder end fill=gray!34,
        minimum width=2.8cm, minimum height=2.4cm, text width=2.4cm,
...
\node[flagnode, left=2.6cm of rule_check] (flag_record) {%
    ...
    {\footnotesize Sets \texttt{inconsistency\_flag}\\ to \texttt{True} and writes an audit note}};
\node[dbnode, below=1.4cm of flag_record] (db) {%
    \textbf{Audit Ledger}\\[2pt]
    {\footnotesize signed, append-only metadata}};
...
\draw[->, thick] (rule_check.south) |- node[edgelabel, near end, above] {Valid} (db.east);
