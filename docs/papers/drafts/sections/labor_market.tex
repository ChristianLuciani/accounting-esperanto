...
hypothesis. The CRA operates on the population of cases that automated
tiers cannot resolve. As stated in Section~\ref{subsec:cra_as_error_management}:
``as the ontology expands --- more jurisdictions mapped, more transaction
types covered, more boundary predicates encoded --- the population of
out-of-coverage cases shrinks and the human review load decreases.'' If
Kontablo's deterministic coverage expands without bound, the CRA residual
converges to zero. A fully converged ontology would require less, not more,
high-level human review.
