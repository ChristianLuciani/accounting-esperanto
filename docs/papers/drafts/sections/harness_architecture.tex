...
\begin{enumerate}
    ...
    below the threshold (Section~\ref{sec:deterministic}), it is flagged for human review rather than
    ...
    Co-responsibility Architecture (Section~\ref{sec:governance}) manages this residual by
...
This relocation framing is corroborated by independent work. The
\emph{inverse parametric knowledge effect} (Section~\ref{subsec:ipke}) places the
value of ontological grounding highest precisely at the coverage boundary, where
LLM parametric knowledge is weakest --- exactly the observable error behavior the
CRA is designed to capture, not suppress. The neuro-symbolic financial reasoning
architecture of Agand (2026) similarly finds that shifting from probabilistic
retrieval to deterministic grounding compresses financial hallucination to 1.2\%,
with residual error concentrating at domain boundaries --- confirming the
relocation pattern as a general property of constrained-output financial systems,
...
\begin{itemize}
    ...
    \item The Three-Tier Resolution Pipeline (Section~\ref{sec:deterministic}), which maximizes
    the proportion of decisions resolved deterministically before reaching
    the model
    \item The Deterministic Boundary Library (Section~\ref{subsec:boundary_library}), which validates
    even Tier 3 proposals against ontological predicates before ledger write
    \item The Co-responsibility Architecture (Section~\ref{sec:governance}), which governs
...
The Co-responsibility Architecture (ADR-008, Section~\ref{sec:governance}) serves two functions
