...
In the impending Machine-to-Machine (M2M) Agent Economy --- driven by frameworks
like Google's \textit{Agent2Agent} (A2A) and the \textit{Agent Payments Protocol}
(AP2) --- AI agents act as autonomous economic actors that negotiate, authorize,
and execute high-frequency financial payments using cryptographically signed
digital mandates. This section sets out the subledger role Kontablo plays for
them.
...
To let AI agents interact with corporate accounting systems through a stable semantic contract rather than a bespoke database schema, Kontablo exposes a \textbf{Model Context Protocol (MCP)} server interface. As of release v0.3.0 the \emph{deterministic core} of that interface is implemented (\texttt{api/mcp/server.py}, built on the official MCP/FastMCP SDK over the stdio transport), alongside the REST API (\texttt{api/rest}, \texttt{api/src}) and the deterministic gRPC core (\texttt{api/grpc/server.py}). It registers six tools --- \texttt{resolve\_account}, \texttt{get\_account}, \texttt{validate\_balance\_sheet}, \texttt{consolidate\_trial\_balances}, \texttt{get\_node\_fiber} and \texttt{list\_jurisdictions} --- each a graph lookup, a deterministic rule, or arithmetic over the same \texttt{core.harness}\slash\texttt{core.engine} logic that backs the other two faces, with no logic duplicated between them. \emph{No tool calls a language model.} An agent can therefore:
\begin{itemize}
    \item Query the current semantic structure of the corporate chart of accounts, including the per-jurisdiction preimage (fiber) of any universal node.
    \item Request a deterministic resolution for a local account code or name, receiving the tier that answered, the exact rule identifier that fired, and any Co-responsibility boundary flags.
    \item Validate a draft trial balance or balance sheet, and consolidate subsidiaries with intercompany elimination, prior to ledger write-in.
\end{itemize}
The honesty bar here is the one applied to gRPC: this is the deterministic core, \emph{not} feature parity with REST. In particular the Tier-3 semantic fallback is deliberately \textbf{not} registered as a tool. Exposing a stochastic answer through an interface that looks deterministic would misrepresent the confidence of a mapping to an agent that is deciding autonomously; an unresolved mapping returns \texttt{resolved=false} and escalates to human review rather than guessing. Tools that would depend on Tier-3 remain planned rather than shipped.
By utilizing the MCP standard, any agent equipped with a compatible client can obtain deterministic mappings and validation results through structured tool calls, without learning the custom relational database schema of the underlying ERP; it interacts entirely with the stable Kontablo Graph abstraction layer. Consistent with the scope boundary of Section~\ref{subsec:enforcement_boundary}, Kontablo is a semantic mapping and validation layer, not a book of record: the agent does not write into a Kontablo-proprietary general ledger. The resulting transaction is recorded in the ERP through the connector --- or, on the roadmap, in a thin UUID-native subledger that synchronizes to an ERP of record --- with Kontablo supplying the validated posting coordinates rather than holding the authoritative ledger itself.
