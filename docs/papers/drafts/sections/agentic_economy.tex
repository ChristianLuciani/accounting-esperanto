...
To let AI agents interact with corporate accounting systems through a stable semantic contract rather than a bespoke database schema, the Kontablo \emph{specification} defines a standardized \textbf{Model Context Protocol (MCP)} server interface. This is design intent, not a shipped feature: as of v0.x the reference implementation exposes its capabilities over a REST API (\texttt{api/rest}, \texttt{api/src}) and a deterministic gRPC core (\texttt{api/grpc/server.py}); the MCP server is specified but not yet implemented, and is tracked as roadmap work. Once shipped, the interface exposes tools that allow LLM agents to:
\begin{itemize}
