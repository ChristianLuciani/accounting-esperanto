...
\begin{abstract}
The proliferation of incompatible national accounting standards creates significant friction in cross-border financial consolidation. Crucially, this ``Accounting Babel'' poses a fatal roadblock for the burgeoning Machine-to-Machine (M2M) Agentic Economy, where autonomous AI agents negotiating across borders require a singular, deterministic financial ledger to record transactions. This paper introduces \textbf{Kontablo}, an open, graph-based universal accounting ontology anchored to IFRS and validated across 195 jurisdictions. Unlike prior work on XBRL taxonomies—which prioritize human-audit disclosures—Kontablo's graph model enables transitive account mappings, multi-hop aggregation rules, and AI-assisted classification. Comparative analysis yields a core taxonomy of 30 Level 3 accounts covering an estimated $\sim$94\% of routine transaction volume by posting count ($\sim$99\% with a 34-account extended core), reproducible from a committed benchmark. We validated this ontology via a mass-consolidation simulation engine across 9 diverse regulatory environments. Kontablo establishes an indispensable financial abstraction layer for ERP interoperability and the nascent Agent Payments Protocol (AP2) ecosystem.
\end{abstract}
...
\section{Conclusion}
Despite global diversity, a \textbf{minimum universal core of 18 accounts} anchors 23 analyzed jurisdictions. Per a reproducible transaction-frequency benchmark (\texttt{scripts/coverage\_benchmark.py}), Kontablo's refined 30 Level 3 accounts cover an estimated $\sim$94\% of routine transaction volume by posting count ($\sim$99\% with a 34-account extended core). Building Kontablo around this abstracted core successfully reconciles legacy commercial ERP fragmentation while enabling a 99\%-automatable, mathematically sound Subledger for the impending Agentic Economy.
